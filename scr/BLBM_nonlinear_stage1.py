...
def funct1(X):
    if len(X.shape) == 2:
        ...
        X1 = X[0]; X2 = X[1]; X3 = X[2]; X4 = X[3]; X5 = X[4]; 
    y = (0.1*np.exp(4*X1) + 4/(1+np.exp(-20*(X2-0.5)))+3*X3+2*X4+X5)/2.5
    return y 
...
def funct2(X):
    if len(X.shape) == 2:
        ...
        X1 = X[0]; X2 = X[1]; X3 = X[2]; X4 = X[3]; X5 = X[4]; 
    y = (0.1*np.exp(4*X1) + 4/(1+np.exp(-20*(X2-0.5)))+3*X3+2*X4+X5)*1.2/2.5
    return y 
...
def sim_mab(seed,n_arms,w_dim,N,percent,n_eval,noise):
    ...
    X_test = np.random.uniform(0,1,n_eval*w_dim).reshape([-1,w_dim])
    rbf_feature = RBFSampler(gamma=1, random_state=1,n_components=n_components)
    X_test_basis = rbf_feature.fit_transform(X_test)
    critical_value = scipy.stats.chi2.ppf(np.sqrt(0.95), X_test_basis.shape[1])
    ...
    for i in range(n_arms):
        ...
        bound_param = w_dim * np.sqrt(np.log(2*w_dim*X_train.shape[0]/np.sqrt(0.975)))
        ...
        MC_quantile = scipy.stats.norm.ppf(1-np.sqrt(0.975), loc=MC_mean, scale=MC_std)
    ...
    regret_mc_non_pessi = cal_regret(est_mc_non_pessi,eval_optimal,X_test,n_eval)
    regret_mc_pessi = cal_regret(est_mc_pessi,eval_optimal,X_test,n_eval)
    regret_mc_quantile = cal_regret(est_mc_quantile,eval_optimal,X_test,n_eval)
    regret_mc_uniform = cal_regret(est_mc_uniform,eval_optimal,X_test,n_eval)
