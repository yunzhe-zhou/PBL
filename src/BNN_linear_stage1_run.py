...
parser.add_argument('-b_size', '--b_size', type=int, default=100)
parser.add_argument('-n_sample', '--n_sample', type=int, default=5)
parser.add_argument('-alpha', '--alpha', type=float, default=0.95)
args0 = parser.parse_args()

string = "bnn_linear_stage1_N_"+str(args0.N) + "_b_size_" + str(args0.b_size) + "_n_sample_" + str(args0.n_sample) + "_alpha_" + str(args0.alpha)
...
def Bayesian_NN(X,y,X_test,N,n_eval):
    ...
    TRAIN_EPOCHS = 500
    SAMPLES = args0.n_sample
    TEST_SAMPLES = args0.N
    BATCH_SIZE = args0.b_size
    ...
    critical_value = scipy.stats.chi2.ppf(np.sqrt(args0.alpha), cov.shape[0])
    ...
    pred_quantile = np.quantile(outputs_arr,q = 1-np.sqrt(1-(1-args0.alpha)/2),axis = 0) #Compute the quantile
...
def sim_mab(seed,n_arms,w_dim,N,percent,n_eval,noise):
    ...
    for i in range(n_arms):
        ...
        est_mc_pessi_i = MC_mean - scipy.stats.norm.ppf(np.sqrt(1-(1-args0.alpha)/2))*MC_std
...
info_ls = []
N_arr = []
for N in [250,500,1000,1500,2000,2500,3000]:
    n_arms = 2
    percent = 0.95
    noise = 0.1
    n_eval = 500
